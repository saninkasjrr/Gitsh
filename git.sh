...
# Function to prompt for Git initialization
prompt_git_init() {
    read -p "No Git repository found. Create new git repository? (y/n): " response
    case $response in
        [yY]|[yY][eE][sS]|"")
            git init
...
# Check if we're in a Git repository, if not, prompt for initialization
